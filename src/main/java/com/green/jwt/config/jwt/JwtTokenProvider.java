...
public class JwtTokenProvider {
    ...
    public String resolveToken(HttpServletRequest req) {
        ...
        if(bearerToken == null || !bearerToken.startsWith(jwtConst.getSchema())) {
            return null;
        }
        //토큰이 있고, Bearer로 문자열이 시작한다. 그러면 Bearer 내용을 제외한 토큰값만 리턴한다.
        //"bearer".length()  >>  6
        //"bearer dsksdalkjsdaljkdsa".substring(3);  >>> d~끝까지
        return bearerToken.substring(jwtConst.getSchema().length() + 1); //Bearer(빈칸)까지 index를 설정해야 하기 때문
    ...
    public Authentication getAuthentication(String token) {
        ...
        // principle : jwtuser 는 사용자 id (username)
        // credentials : pw - 여기서는 찍을 필요 없으니 null 처리
        // jwtuser 의 권한
    }
