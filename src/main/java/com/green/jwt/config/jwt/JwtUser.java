...
@Getter
@EqualsAndHashCode
@RequiredArgsConstructor // equals , hashCode 메소드 오버라이딩
...
//                    }
//                })
                .map(item -> new SimpleGrantedAuthority(item.name())) // map은 똑같은 size 의 스트림을 만든다. Stream<List<SimpleGrantedAuthority>> 으로 변환
                .toList();
